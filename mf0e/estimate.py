...
class F0Estimate:
    ...
    def _stft(self, x, fs):
        ...
        X = np.array(
            [
                np.fft.fft(win * x[i : i + frame_len_samps], K)
    ...
    def _spectral_whitening(self, X, fs, nu=0.33):
        ...
        for b in range(1, len(c_bins) - 1):
            ...
            gamma[:, centre] = np.sqrt(
                (2 / K) * np.sum(H * (np.abs(X[:, :nyquist_bin]) ** 2), axis=1)
            ) ** (nu - 1)

            # interpolate between the previous centre bin and the current centre bin
            # for each STFT frame
            for t in range(T):
                gamma[t, left:centre] = np.linspace(
                    gamma[t, left], gamma[t, centre], centre - left
                )
    ...
    def _iterative_est(self, Y, fs):
        ...
        for t in range(T):
            ...
            while len(salience_hats) < self._max_poly:
                tau_hat, salience_hat, Y_t_D = self._search_smax(
                    Y_t_R, fs, tau_prec=0.5
                )
    ...
    def _search_smax(self, Y_t_R, fs, tau_prec=1.0):
        ...
        salience_hat, harmonics = self._calc_salience(
            Y_t_R, fs, tau_low[q_best], tau_up[q_best]
        )
        K = len(Y_t_R) << 1
    ...
    def _calc_salience(self, Y_t_R, fs, tau_low, tau_up):
        ...
        for m in range(1, num_harmonics + 1):
            ...
            harmonic_bin = (
                np.argmax(Y_t_R[harmonic_lb - 1 : harmonic_ub]) + harmonic_lb - 1
            )
    ...
    def _calc_harmonic_spec(self, fs, K, harmonics):
        ...
        partial_spectrum = np.hstack(
            (
                window_spec[self._partial_width :: -1],
    ...
    def collapse_notes(self, notes):
        ...
        for frame_n in notes:
            ...
            if len(frame_n) != len(prev_frame):
                ...
                [
                    n1["pname"] == n2["pname"] and n1["oct"] == n2["oct"]
